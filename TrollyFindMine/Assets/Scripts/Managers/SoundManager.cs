...
public class SoundManager : MonoBehaviour
{
    ...
    public void FadeAndChange(Define.BGM BGMSound, float fadeTime = 0.3f)
    {
        if (_currentBGM == BGMSound)
            return;

        AudioSource bgmSource = _audioSources[(int)Define.Sounds.BGM];

        _bgmFadeTween?.Kill();
        _bgmFadeTween = bgmSource.DOFade(0f, fadeTime).OnComplete(() =>
        {
            Play(BGMSound);
            bgmSource.volume = 0f;
            bgmSource.DOFade(BGMVolume, fadeTime);
        });
    }
}